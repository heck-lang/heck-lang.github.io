...
function init(wabt) {
	...
	function heck(buffer) {
		...
		// load a heck string to js
		function heck_js_str(mem, str) {
		...
		let heck_memory = new WebAssembly.Memory({ initial: 2 });
		...
		const imports = {
			...
			_print_string: function (val) {
				console.log(heck_js_str(heck_memory, val));
			},
			...
			input: function () {
				...
				alloc_pos = create_str(heck_memory, alloc_pos, prompt());
				return str_pos;
			},
			parseInt: function (str) {
				return parseInt(heck_js_str(heck_memory, str));
			},
			parseFloat: function (str) {
				return parseFloat(heck_js_str(heck_memory, str));
			},
			_concat_int_l: function (a, b) {
				let str_pos = alloc_pos;
				let str = heck_js_str(heck_memory, b);
				alloc_pos = create_str(heck_memory, alloc_pos, a + str);
				return str_pos;
			},
			_concat_int_r: function (a, b) {
				let str_pos = alloc_pos;
				let str = heck_js_str(heck_memory, a);
				alloc_pos = create_str(heck_memory, alloc_pos, str + b);
				return str_pos;
			},
			...
			_concat_bool_l: function (a, b) {
				...
				let str = heck_js_str(heck_memory, b);
				alloc_pos = create_str(heck_memory, alloc_pos, bool + str);
				return str_pos;
			},
			_concat_bool_r: function (a, b) {
				...
				alloc_pos = create_str(heck_memory, alloc_pos, str + bool);
				return str_pos;
			},
			_concat_str: function (a, b) {
				let str_pos = alloc_pos;
				let str_a = heck_js_str(heck_memory, a);
				let str_b = heck_js_str(heck_memory, b);
				alloc_pos = create_str(heck_memory, alloc_pos, str_a + str_b);
				return str_pos;
			},
			...
			intArray: function (size) {
				...
				alloc_pos = create_arr(heck_memory, alloc_pos, size);
				return arr_pos;
			},
			intArray2d: function (width, height) {
				let arr_pos = alloc_pos;
				alloc_pos = create_arr_2d(heck_memory, alloc_pos, width, height);
				return arr_pos;
			},
			...
			_string_len: function (str) {
				return arr_len(heck_memory, str);
			},
			_intArray_len: function (arr) {
				return arr_len(heck_memory, arr);
			},
		...
		// add user imports
		if (heckImports != null) {
			let finalImports = Object.assign(imports, heckImports);
		}
	...
	function run() {
		...
		try {
			...
			console.log("unable to run code. Heck!")
		}
	}
